        ...
        <div className="grid grid-cols-1 md:grid-cols-2">
          ...
          <div className="hidden md:flex flex-col justify-center p-10 bg-gradient-to-br from-bank-primary via-[#1a3a6b] to-bank-dark text-white relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <h2 className="text-2xl font-bold mb-3">{s.site_short_name || t('企业门户')}<br/>{t('信息门户')}</h2>
              <p className="text-sm text-blue-200 leading-relaxed mb-8">{s.site_description || `${s.site_short_name || t('企业门户')} ${t('信息门户平台')}`}</p>
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center"><FileText size={14} /></div>
                  <div><p className="text-sm font-medium">{t('评论互动')}</p><p className="text-[10px] text-blue-200">{t('发表评论，收藏支持文章')}</p></div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center"><Users size={14} /></div>
                  <div><p className="text-sm font-medium">{t('意见征集')}</p><p className="text-[10px] text-blue-200">{t('参与建议征集和反馈')}</p></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-8 md:p-10">
            <div className="mb-8">
              <h1 className="text-xl font-bold text-gray-800 dark:text-gray-100">{t('用户登录')}</h1>
              <p className="text-xs text-gray-400 mt-1">{t('请输入您的账号密码进行登录')}</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1.5">{t('用户名')}</label>
                <input
                  type="text"
                  value={form.username}
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1.5">{t('密码')}</label>
            ...
            <div className="flex items-center gap-3 mt-6">
              ...
              <span className="text-[10px] text-gray-400">{t('其他选项')}</span>
              <div className="h-px bg-gray-200 dark:bg-gray-700 flex-1" />
            </div>

            <p className="text-center text-xs text-gray-500 mt-4">
              {t('还没有账号？')}<Link href="/register" className="text-bank-red hover:underline ml-1 font-medium">{t('立即注册')}</Link>
            </p>
          </div>
        </div>
      </div>
